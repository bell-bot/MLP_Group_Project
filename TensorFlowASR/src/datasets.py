...
data_paths = "/home/szy/Documents/code/espnet/egs/tedlium3/asr1/db"
...
def make_metadata_csv(ted_object,output_csv_path,csv_file,keys):
    full_path = output_csv_path + csv_file
    print("-"*20)
    print("Writing to: "+csv_file)
    with open(full_path, mode='w') as metadata:
        metadata_writer = csv.writer(metadata, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)

        for i in tqdm(range(ted_object.__len__())):
            item = ted.__getitem__(i)
            row = []
            for key in keys:
                row.append(str(item.__getitem__(key)).strip())
            metadata_writer.writerow(row)
    print("-"*20)
    print("Done! Located at: "+full_path)
...
if __name__== "__main__":
    ...
    #x= TED_wrapper(path_to_labels_csv = './Data/KeywordPerSample/labels_keywords.csv')
    #output_df = x.get(4)
    #print(output_df)
    ...
    ted = TEDLIUMCustom()
    print(ted.__len__())
    print(ted.__getitem__(1))
    output_csv_path = "/home/szy/Documents/MLP_Group_Project/src/"
    csv_name = "metadata.csv"
    keys = ["talk_id","start_time","end_time","transcript",]
    make_metadata_csv(ted,output_csv_path,csv_name,keys)
