...
from src.datasets import MultiLingualSpokenWordsEnglish
...
keyword_path = MultiLingualSpokenWordsEnglish.MSWC_EN_AUDIO_FOLDER + "/"
...
def read_ctrlf_dataset(start_sample= 0, num_of_samples=3000, if_keyword=False):
    ...
    for i in tqdm(range(start_sample,num_of_samples),desc="Preparing Dataset"):
        ...
        if len(sample) ==0:
            num_of_samples+=1
            continue
        else: #TODO: Go through each keyword
            ...
            ted_talk_id = sample["TED_talk_id"][0]

            keyword_id = sample["MSWC_ID"][0]
            row = None
            if if_keyword:
                row = [keyword_id, sample["keyword"][0]]
            else:
...
def encode_single_sample(opus_file, label):
    ###########################################
    ##  Process the Audio
    ##########################################
     # 1. Read wav file
    file = tf.io.read_file(opus_file)
    # 2. Decode the wav file
    audio, _ = tf.audio.decode_vorbis(file)
...
metadata_keyword_df = read_ctrlf_dataset(start_sample= TEST_START, num_of_samples=TEST_END, if_keyword=True)
print(metadata_keyword_df[0:10])
# df_train = metadata_df[:split]
df_val_keyword = metadata_keyword_df
# print(f"Size of the training set: {len(df_train)}")
print(f"Size of the test set: {len(df_val_keyword)}")
# The set of characters accepted in the TED_transcription.
...
# Define the validation dataset
validation_keyword_dataset = tf.data.Dataset.from_tensor_slices(
    (list(df_val_keyword["TED_Talk_ID"]), list(df_val_keyword["TED_transcript"]))
)
validation_keyword_dataset = (
    validation_keyword_dataset.map(encode_single_sample, num_parallel_calls=tf.data.AUTOTUNE)
    .padded_batch(BATCH_SIZE)
    .prefetch(buffer_size=tf.data.AUTOTUNE)
)
...
validation_keyword_dataset = validation_keyword_dataset.with_options(options)
...
def eval_model(num_of_predictions_to_print=5):
    ...
    with open(model_directory + "model_ted_stats_test.txt", "a") as f:  
        print("-" * 100)
...
def eval_model_keywords(num_of_predictions_to_print=5):
    
    # Let's check results on more validation samples
    predictions = []
    targets = []
    for batch in validation_keyword_dataset:
        X, y = batch
        batch_predictions = model.predict(X)
        batch_predictions = decode_batch_predictions(batch_predictions)
        predictions.extend(batch_predictions)
    ...
    with open(model_directory + "model_keywords_stats_test.txt", "a") as f:  
        print("-" * 100)
        print(f"Word Error Rate: {wer_score:.4f}")
        f.write("WER: " + str(wer_score) +"\n")
        print("-" * 100)
...
# eval_model()
# print((start_time_perf - time.perf_counter())/len(validation_dataset))
